...
public class ProductController {
    ...
    @PostMapping("/detail")
    public R getDetails(@RequestBody @Validated ProductIdParam productIdParam ,BindingResult result) {

          if(result.hasErrors()) {
              return R.fail("商品详情查询失败");
          }
          return productService.getDetails(productIdParam.getProductID());
    }


    @PostMapping("/pictures")
    public R getPictures(@RequestBody @Validated ProductIdParam productIdParam ,BindingResult result) {

        if(result.hasErrors()) {
            return R.fail("商品图片详情查询失败");
        }
        return productService.getPictures(productIdParam.getProductID());
    }
